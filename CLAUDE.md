...
8. Enforces `MAX_DAILY_SCRAPES = 180` hard cap and `PIPELINE_DEADLINE_SECONDS = 1800` wall-clock deadline (keeps runtime inside the GitHub Actions 40-min job timeout), with a **protected tail budget**: once remaining scrape slots fall to the derived tail demand (`_tail_scrape_demand` — sum of `results_per_entity` over concept targets; 38 with current config) or remaining wall-clock to `TAIL_RESERVE_SECONDS = 360`, remaining *entity* targets are skipped so the concept/macro groups at the bottom of `targets.yaml` always get their discovery pass (concept targets keep running until the hard limits fire)
